...
async function buildPlan(prompt: string): Promise<DocPlan> {
      ...
      messages: [
        {
          role: "system",
          content:
            "Devuelve JSON con title, summary, sections[{heading, bullets[]}], y opcional table{headers, rows}. Crea contenido claro, útil y breve en español.",
        },
  ...
  const title =
    typeof parsed.title === "string" && parsed.title.trim() ? parsed.title.trim() : "Documento";
    ...
    parsed.table && Array.isArray(parsed.table.headers) && Array.isArray(parsed.table.rows)
      ? {
          ...
          rows: parsed.table.rows
            ...
            .map((row) =>
              row.filter((cell): cell is string => typeof cell === "string").slice(0, 5),
            )
...
async function createPptx(plan: DocPlan) {
  ...
  titleSlide.addText(plan.summary, {
    x: 0.6,
    y: 1.9,
    w: 9.1,
    h: 1.2,
  ...
  plan.sections.slice(0, 4).forEach((section) => {
    ...
    slide.addText(
      section.bullets.map((bullet) => ({ text: bullet, options: { bullet: { indent: 18 } } })),
      {
        x: 0.8,
        y: 1.3,
        w: 8.4,
...
export const generateDocument = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: GenerateDocInput) => {
    ...
    if (
      ...
      input.prompt.trim().length < 6 ||
      input.prompt.length > 3000
    ) {
      throw new Error("Describe mejor el documento");
    }
    if (!input.themeName || !["NEVIRA", "NOVA"].includes(input.themeName))
